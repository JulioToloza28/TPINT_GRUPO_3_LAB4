							...
							<%
								if (listaMateria != null)
									for (Materia mate : listaMateria) {
										if (curso.getIdMateria() == mate.getId()) {
							%>
							...
							<%
								if (listaTurnos != null)
									for (Turno turno : listaTurnos) {
										if (turno.getIdTurno() == curso.getIdTurno()) {
							%>
						...
						<label for="sel1">Año:</label> <select class="form-control"
							...
							<%
								for (int x = 2020; x >= 2000; x--) {
									if (curso.getAnio() == x) {
							%>
							<option selected value=<%=curso.getAnio()%>><%=curso.getAnio()%></option>
							<%
								} else {
							%>
							<option><%=x%></option>
						...
						<%
							if (listaProfesor != null)
								for (Profesor prof : listaProfesor) {
									if (prof.getLegajo() == curso.getLegajoProf()) {
						%>
